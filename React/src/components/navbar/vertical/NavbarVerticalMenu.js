  ...
  return routes.map((route) => {
    if (!route.children) {
      return (
        <Nav.Item
          as="li"
          key={route.name + Math.random(100)}
          onClick={handleNavItemClick}
        >
          <NavLink
            end={route.exact}
            to={route.path}
            state={{ open: route.path === "/authentication-modal" }}
            className={({ isActive }) =>
              isActive && route.path !== "#!" ? "active nav-link" : "nav-link"
            }
          >
            <NavbarVerticalMenuItem route={route} />
          </NavLink>
    ...
    return <CollapseItems route={route} key={route.name + Math.random(100)} />;
  });
};
